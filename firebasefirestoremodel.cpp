    ...
    : QObject(parent)
{
    ...
    QAndroidJniObject obj(JavaLangObject::toChar(FirebaseFirestoreModel::kJavaClassName));
     qDebug() << kTag << "pre java register";
    obj.callMethod<void>("register");
    qDebug() << kTag << "java registered";


    QtChannel * channel = QtChannel::instance();
    QObject::connect(channel, &QtChannel::messageReceived,
...
void FirebaseFirestoreModel::setMessage(QtChannelMessage &message){

    qDebug() << kTag << "setMessage ";
    qDebug() << kTag << "dump: " << message.dump();

    if(message.hasError()){
        emit FirebaseFirestoreModel::error(message.getErrorDescription());
    }else{
        ...
        if(message.getMethodName() == this->kMethodGetCollection){
            ...
            qDebug() << kTag << "message method not found";
        }
    }
}

void FirebaseFirestoreModel::getCollection(QString const &collection){
    qDebug() << kTag << "getCollection";
    QtChannel* channel = QtChannel::instance();
    QtChannelMessage* message = QtChannelMessage::newJavaObject(this->kChannelName, this->kMethodGetCollection);
    message->withArg(collection);
    channel->callMethod(*message);
}

void FirebaseFirestoreModel::init(){
    qDebug() << kTag << "init";
    QtChannel * channel = QtChannel::instance();
    QtChannelMessage* message = QtChannelMessage::newJavaObject(this->kChannelName, this->kMethodInit);
    channel->callMethod(*message);
}
